...
const Item = ({ifExists,id,color,shadow,title,text,img,btn,rating,price}) => {
  ...
  const addToCart=()=>{
    const item={id,title,text,price,color,img,shadow}
  ...
  return (
    ...
    <div className={`grid items-center ${ifExists? 'justify-items-start':'justify-items-center'}`}>
      ...
      <div className='flex items-center gap-3'>
        <button type='button' onClick={()=> addToCart()} className='bg-white opacity-90 blur-effect-theme button-theme p-0.5 shadow shadow-sky-200'><ShoppingBagIcon className='icon-style text-slate-900'/></button>
        <button type='button' className='bg-white opacity-90 blur-effect-theme button-theme px-2 py-1 shadow shadow-sky-200 text-sm text-black'>{btn} </button>
      </div>
    </div>
    <div className={`flex items-center ${ifExists?'absolute top-5 right-1':'justify-center'}`}>
      <img src={img} alt={`img/sneakers/${id}` }className={`transitions-theme hover:-rotate-12 ${ifExists? 'h-auto w-64 lg:w-56 md:w-48 -rotate-[35deg]': 'h-36 w-64'}`} />
    </div>
    </div>
    </>
  )
